...
internal static class Handler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var columns = await DbIntrospection.GetColumnNamesAsync(connectionFactory, "Tipologica", "TipoOrganizzazione", cancellationToken);
        var set = new HashSet<string>(columns, StringComparer.OrdinalIgnoreCase);

        var idCandidates = new[] { "TipoOrganizzazioneId", "Id" };
        ...
        {
            // Unexpected schema: surface as 404 so UI can fallback gracefully.
            return null;
        }
        ...
        var sql = $"""
            ...
            FROM [Tipologica].[TipoOrganizzazione]
            {whereSql}
            ORDER BY [Label];
            """;
